...
- **高鲁棒的判别大脑**：成功解决 PRM 训练中的冷坍缩（Cold Collapse）难题，使价值头能够精准识别沙箱级错误。
...
训练全程采用**稀疏的结果导向奖励（Outcome Reward）**，只要整条执行轨迹的最终答案与 Ground Truth 匹配，该轨迹的所有动作都将获得正向 Advantage。
...
裁判大脑实时对 Actor 生成的每一个候选步骤进行打分：遇到低置信度分支（如 `score < 0.1`）立即剪枝并回溯；遇到高置信度分支（如 `score > 0.8`）则顺藤摸瓜，继续深入调用沙箱。真正实现了用“Test-time Compute”换取准确率。
